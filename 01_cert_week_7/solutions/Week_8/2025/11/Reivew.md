...
- 문제7풀이법 활용 괜찮은듯 다음 풀이 때 자세한 복기 함더


```aiignore
전체적으로 복기

1. 3D 테이블 - 테이블을 어떻게 붙여야 하는지, 불필요한 테이블은 없는지
2. 상상 출력 - 정상적으로 나올지, NULL이나 다른 값이 들어오거나 값이 빠지는 경우를 검증(가설 검증)
3. 인간 코딩 - 코덱스보다 깔끔하게 코딩하는지(리팩토링)
